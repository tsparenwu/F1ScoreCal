...
class SimpleSimilarityCalculator:
    ...
    def calculate_sentence_similarity(self, sent1, sent2):
        ...
        """
        # 預處理文本
        processed_sent1 = self.preprocess_text(sent1)
        processed_sent2 = self.preprocess_text(sent2)
        
        # 如果句子為空，返回0
        if not processed_sent1.strip() or not processed_sent2.strip():
            return 0.0
        
        try:
            # 使用TF-IDF向量化
            tfidf_matrix = self.vectorizer.fit_transform([processed_sent1, processed_sent2])
            ...
            # 如果計算失敗，使用簡單的詞重疊相似度
            words1 = set(processed_sent1.split())
            words2 = set(processed_sent2.split())
