...
def calculate_similarity():
    ...
    try:
        ...
        calc = get_calculator()
        if calc is None:
            return jsonify({'error': 'BERT模型初始化失敗，請檢查依賴是否正確安裝'}), 500
        ...
        # 格式化結果
        formatted_result = {
            'f1_score': float(round(result['f1_score'], 4)),
            ...
            'matched_pairs': [
                {
                    ...
                    'similarity': float(round(pair['similarity'], 4)),
                    'sentence1_idx': int(pair['sentence1_idx']),
                    'sentence2_idx': int(pair['sentence2_idx'])
                }
            ...
            'sentence_similarities': [
                {
                    ...
                    'similarity': float(round(sim['similarity'], 4)),
                    'sentence1_idx': int(sim['sentence1_idx']),
                    'sentence2_idx': int(sim['sentence2_idx'])
                }
                for sim in result['sentence_similarities']
            ],
            'weights1': [float(round(w, 4)) for w in result['weights1']],
            'weights2': [float(round(w, 4)) for w in result['weights2']],
...
def health_check():
    ...
    try:
        calc = get_calculator()
        if calc is None:
            return jsonify({'status': 'error', 'message': 'BERT模型未初始化'}), 500
...
if __name__ == '__main__':
    print("正在啟動BERT文本相似度計算服務...")
    print("首次運行可能需要下載BERT模型，請耐心等待...")
    ...
    try:
        ...
        print(f"模型初始化失敗: {e}")
        print("請確保已安裝所有依賴: pip3 install -r requirements.txt")
